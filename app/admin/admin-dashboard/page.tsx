...
// Types
interface User { id?:number|string; full_name?:string; username?:string; email?:string; role?:string; profile_picture?:string; company_id?:number|string; company_name?:string; avatar_color?:string; }
interface TeamMember { id:number|string; full_name?:string; name?:string; username?:string; role?:string; job_role?:string; status?:'office'|'remote'|'away'; note?:string; since?:string; profile_picture?:string; bg?:string; avatar_color?:string; }
...
const S = {
  ...
  logo: {padding:'18px 16px 12px',display:'flex',alignItems:'center',gap:8,borderBottom:'1px solid #f0ede8'},
  ...
  lbl: {fontSize:11,fontWeight:700,color:'#6b6860',display:'block',marginBottom:4},
};
...
export default function AdminDashboard() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try { const r=await axios.get(`${API}/api/companies/applications`); if(r.data?.success) setApplications(r.data.applications||[]); } catch {}
  }, [user?.company_id]);

  const fetchAssignUsers = useCallback(async () => {
    try { const r=await axios.get(`${API}/api/tasks/users`); setAssignUsers(r.data?.data?.users||r.data?.users||[]); } catch {}
  }, []);
  ...
  const handleAssignTask = async (d:{title:string;description:string;assigned_to:string;priority:string;due_date:string}) => {
    setTaskLoading(true);
    try { await axios.post(`${API}/api/tasks`,d); showToast('Task assigned!'); setShowTask(false); fetchTasks(); }
    catch { showToast('Failed','error'); }
    finally { setTaskLoading(false); }
  };

  const handleApplication = async (id:number|string, status:'accepted'|'rejected') => {
    try { await axios.post(`${API}/api/companies/handle-application`,{applicationId:id,status}); showToast(`Application ${status}!`); fetchApplications(); fetchTeam(); }
    catch { showToast('Failed','error'); }
  };
    ...
    <div style={{display:'flex',height:'100vh',overflow:'hidden',background:'#faf9f7',fontFamily:'Outfit,sans-serif'}}>
      ...
      <aside style={S.sidebar}>
        <div style={S.logo}>
          ...
          <span style={{marginLeft:'auto',fontSize:9,fontWeight:900,padding:'2px 7px',borderRadius:100,background:'linear-gradient(135deg,#f97316,#ef4444)',color:'#fff',letterSpacing:'.05em'}}>ADMIN</span>
        </div>
        ...
        <nav style={{padding:'8px',flex:1,overflowY:'auto'}}>
          ...
          <div style={{height:1,background:'#f0ede8',margin:'8px 4px'}}/>
          <button onClick={()=>router.push('/admin/jobs')} style={S.navItem(false)}>
            <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" strokeLinejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
            Post Job
          </button>
        ...
        <div style={{padding:'10px',borderTop:'1px solid #f0ede8'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderRadius:10,background:'#faf9f7',marginBottom:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:10,color:'#9e9b94'}}>{user?.company_name||'Admin'}</div>
            </div>
          </div>
          <button onClick={logout} style={{width:'100%',textAlign:'left',padding:'7px 10px',fontSize:11,fontWeight:600,display:'flex',alignItems:'center',gap:8,background:'none',border:'none',color:'#9e9b94',cursor:'pointer',borderRadius:8,fontFamily:'Outfit,sans-serif',transition:'all .15s'}}>
      ...
      <main style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'24px 28px'}}>
          ...
          {nav==='overview'&&(
            <div style={{display:'flex',flexDirection:'column',gap:20}}>
              ...
              <div style={{background:'linear-gradient(135deg,#0f0e0c,#2d2b28)',color:'#fff',padding:'28px 32px',borderRadius:20,position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',top:-40,right:-40,width:160,height:160,background:'#f97316',borderRadius:'50%',filter:'blur(60px)',opacity:.2}}/>
                <div style={{position:'relative',zIndex:1}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                    <span style={{fontSize:10,fontWeight:900,letterSpacing:'.1em',textTransform:'uppercase',background:'#f97316',color:'#fff',padding:'3px 10px',borderRadius:100}}>Welcome Back</span>
                  ...
                  <p style={{fontSize:13,opacity:.6,margin:'0 0 16px'}}>{team.filter(m=>m.status==='office').length} in office � {tasks.filter(t=>t.status!=='done').length} active tasks</p>
                  <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                    ...
                    <button onClick={()=>setShowTask(true)} style={{...S.btn,background:'rgba(255,255,255,.15)',color:'#fff',fontSize:12,padding:'8px 16px'}}>+ Assign Task</button>
                  </div>
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:12}}>
                {[
                  {label:'In Office',value:team.filter(m=>m.status==='office').length,icon:'??',color:'#f97316',bg:'rgba(249,115,22,.08)'},
                  {label:'Remote',value:team.filter(m=>m.status==='remote').length,icon:'??',color:'#60a5fa',bg:'rgba(96,165,250,.08)'},
                  {label:'Active Tasks',value:tasks.filter(t=>t.status!=='done').length,icon:'??',color:'#ef4444',bg:'rgba(239,68,68,.08)'},
                  {label:'Done Today',value:tasks.filter(t=>t.status==='done').length,icon:'?',color:'#22c55e',bg:'rgba(34,197,94,.08)'},
                ].map(s=>(
                  <div key={s.label} style={{...S.card,padding:'16px 18px'}}>
                    ...
                    <div style={{fontSize:24,fontWeight:900,color:'#0f0e0c'}}>{s.value}</div>
                  </div>
                ))}
              </div>
              ...
              {applications.length>0&&(
                <div>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                    <div><div style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:900}}>Pending Applications</div><div style={{fontSize:11,color:'#9e9b94'}}>{applications.length} waiting</div></div>
                    <button onClick={()=>router.push('/admin/applications')} style={{...S.btnGhost,fontSize:11,padding:'6px 12px'}}>View All ?</button>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:10}}>
                    {applications.slice(0,4).map(app=>(
                      <div key={app.id} style={{...S.card,padding:'14px 16px',display:'flex',alignItems:'center',gap:10}}>
                        <Av name={app.full_name||app.username} size={36}/>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:13,fontWeight:700,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{app.full_name||app.username}</div>
                          <div style={{fontSize:11,color:'#9e9b94',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{app.email}</div>
                        </div>
                        <div style={{display:'flex',gap:6}}>
                          <button onClick={()=>handleApplication(app.id,'accepted')} style={{width:30,height:30,borderRadius:8,border:'none',background:'rgba(34,197,94,.1)',color:'#16a34a',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>?</button>
                          <button onClick={()=>handleApplication(app.id,'rejected')} style={{width:30,height:30,borderRadius:8,border:'none',background:'rgba(239,68,68,.1)',color:'#ef4444',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>?</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                team.map(m=>(
                  <div key={m.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderBottom:'1px solid #f0ede8'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:11,color:'#9e9b94'}}>{m.job_role||m.role||'Member'}</div>
                    </div>
          ...
          {nav==='tasks'&&(
            <div>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                <div><div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:900}}>Tasks</div><div style={{fontSize:11,color:'#9e9b94'}}>{tasks.filter(t=>t.status!=='done').length} open � {tasks.filter(t=>t.status==='done').length} done</div></div>
                <button onClick={()=>setShowTask(true)} style={{...S.btn,fontSize:11,padding:'7px 14px'}}>+ Assign Task</button>
              </div>
              {tasks.length===0?<div style={{textAlign:'center',padding:'48px 24px',background:'#fff',border:'1.5px dashed #e5e5e5',borderRadius:16}}><div style={{fontSize:40,marginBottom:12}}>??</div><div style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:900,marginBottom:6}}>No tasks yet</div><div style={{fontSize:12,color:'#9e9b94'}}>Assign your first task to a team member</div></div>:
                ...
                {tasks.map(t=>(
                  <div key={t.id} style={{...S.card,padding:'14px 18px',display:'flex',alignItems:'center',gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:11,color:'#9e9b94'}}>? {t.assigned_to_name||'Team member'}</div>
                    </div>
                    <span style={{fontSize:10,fontWeight:700,padding:'3px 9px',borderRadius:100,background:{todo:'#f0ede8',in_progress:'rgba(249,115,22,.1)',done:'rgba(34,197,94,.1)'}[t.status],color:{todo:'#9e9b94',in_progress:'#d45e00',done:'#16a34a'}[t.status]}}>
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose()}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:9000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:'#fff',borderRadius:20,padding:24,width:'100%',maxWidth:420}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
          <span style={{fontFamily:'Syne,sans-serif',fontWeight:900,fontSize:16}}>Assign Task</span>
          <button onClick={onClose} style={{width:28,height:28,borderRadius:'50%',border:'1px solid #e5e5e5',background:'#f8f8f8',cursor:'pointer',fontSize:14}}>�</button>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <div><label style={{fontSize:11,fontWeight:700,color:'#6b6860',display:'block',marginBottom:4}}>Assign to *</label>
            <select style={{width:'100%',border:'1.5px solid #e5e5e5',borderRadius:10,padding:'8px 12px',fontSize:12,fontFamily:'Outfit,sans-serif',outline:'none',boxSizing:'border-box' as const}} value={assignedTo} onChange={e=>setAssignedTo(e.target.value)}>
              <option value="">Select member�</option>
              {users.map(u=><option key={u.id} value={String(u.id)}>{u.full_name||u.username}</option>)}
            </select>
          </div>
          <div><label style={{fontSize:11,fontWeight:700,color:'#6b6860',display:'block',marginBottom:4}}>Title *</label>
            <input style={{width:'100%',border:'1.5px solid #e5e5e5',borderRadius:10,padding:'8px 12px',fontSize:12,fontFamily:'Outfit,sans-serif',outline:'none',boxSizing:'border-box' as const}} placeholder="Task title" value={title} onChange={e=>setTitle(e.target.value)}/>
          </div>
          <div><label style={{fontSize:11,fontWeight:700,color:'#6b6860',display:'block',marginBottom:4}}>Priority</label>
            <div style={{display:'flex',gap:6}}>
              {(['low','medium','high'] as const).map(p=><button key={p} onClick={()=>setPriority(p)} style={{flex:1,padding:'6px',borderRadius:8,border:`1.5px solid ${priority===p?'#f97316':'#e5e5e5'}`,background:priority===p?'rgba(249,115,22,.1)':'transparent',cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:'Outfit,sans-serif',color:priority===p?'#f97316':'#9e9b94',textTransform:'capitalize'}}>{p}</button>)}
            </div>
          </div>
          <div><label style={{fontSize:11,fontWeight:700,color:'#6b6860',display:'block',marginBottom:4}}>Due Date</label>
            <input type="date" style={{width:'100%',border:'1.5px solid #e5e5e5',borderRadius:10,padding:'8px 12px',fontSize:12,fontFamily:'Outfit,sans-serif',outline:'none',boxSizing:'border-box' as const}} value={dueDate} onChange={e=>setDueDate(e.target.value)}/>
          </div>
        </div>
        <div style={{display:'flex',gap:8,marginTop:16}}>
          <button onClick={onClose} style={{flex:1,padding:'9px',borderRadius:10,border:'1.5px solid #e5e5e5',background:'#fff',cursor:'pointer',fontFamily:'Outfit,sans-serif',fontWeight:700,fontSize:12}}>Cancel</button>
          <button disabled={!title.trim()||!assignedTo||loading} onClick={()=>onSubmit({title,description:'',assigned_to:assignedTo,priority,due_date:dueDate})} style={{flex:2,padding:'9px',borderRadius:10,border:'none',background:'#0f0e0c',color:'#fff',cursor:'pointer',fontFamily:'Outfit,sans-serif',fontWeight:700,fontSize:12,opacity:(!title.trim()||!assignedTo)?0.5:1}}>
...
function ManageSection({user,showToast}:{user:User|null;showToast:(m:string,t?:'success'|'error')=>void}) {
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div><div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:900}}>Manage Team</div><div style={{fontSize:11,color:'#9e9b94'}}>{members.length} members</div></div>
        <button onClick={fetch} style={{background:'#fff',color:'#0f0e0c',border:'1px solid #e5e5e5',borderRadius:10,padding:'6px 12px',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Outfit,sans-serif'}}>Refresh</button>
      </div>
      {loading?<div style={{textAlign:'center',padding:40,color:'#9e9b94',fontSize:13}}>Loading�</div>:members.length===0?<div style={{textAlign:'center',padding:'48px 24px',background:'#fff',border:'1.5px dashed #e5e5e5',borderRadius:16}}><div style={{fontSize:40,marginBottom:12}}>??</div><div style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:900,marginBottom:6}}>No members yet</div><div style={{fontSize:12,color:'#9e9b94'}}>Accept applications to add members</div></div>:
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {members.map((m,i)=>(
          <div key={m.id} style={{background:'#fff',border:'1px solid #f0ede8',borderRadius:14,padding:'12px 16px',display:'flex',alignItems:'center',gap:12,animationDelay:`${i*.04}s`}}>
            <Av name={m.full_name||m.username} pic={m.profile_picture} size={36}/>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:2}}>
                <span style={{fontSize:13,fontWeight:700}}>{m.full_name||m.username}</span>
                <span style={{fontSize:9,fontWeight:900,padding:'1px 7px',borderRadius:100,background:m.role==='admin'?'linear-gradient(135deg,#f97316,#ef4444)':'#f0ede8',color:m.role==='admin'?'#fff':'#9e9b94'}}>{m.role}</span>
              </div>
              <div style={{fontSize:11,color:'#9e9b94'}}>{m.email}</div>
            </div>
            {String(m.id)!==String(user?.id)&&<button onClick={()=>remove(m.id,m.full_name||m.username||'User')} disabled={removing===m.id} style={{padding:'5px 12px',borderRadius:8,border:'none',background:'rgba(239,68,68,.1)',color:'#ef4444',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'Outfit,sans-serif'}}>{removing===m.id?'�':'Remove'}</button>}
          </div>
        ))}
      </div>}
    </div>
...
function AnalyticsSection() {
  ...
  const [taskStats,setTaskStats] = useState<{total:number;todo:number;in_progress:number;done:number;overdue:number}|null>(null);
  useEffect(()=>{
    axios.get(`${API}/api/attendance/analytics`).then(r=>{const d=r.data?.data;if(d){setStats(d.today);setWeekly(d.daily||[]);}}).catch(()=>{});
    axios.get(`${API}/api/tasks`).then(r=>{const tasks=r.data?.data?.tasks||r.data?.data||r.data||[];if(Array.isArray(tasks)){setTaskStats({total:tasks.length,todo:tasks.filter((t:{status:string})=>t.status==='todo').length,in_progress:tasks.filter((t:{status:string})=>t.status==='in_progress').length,done:tasks.filter((t:{status:string})=>t.status==='done').length,overdue:0});}}).catch(()=>{});
  },[]);
  const maxVal = Math.max(...weekly.map(d=>d.office+d.remote+d.away),1);
  return (
    <div style={{display:'flex',flexDirection:'column',gap:20}}>
      <div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:900}}>Analytics</div>
      {/* Attendance stats */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:12}}>
        {[
          {label:'In Office',value:stats?.in_office||0,color:'#f97316',bg:'rgba(249,115,22,.08)'},
          {label:'Remote',value:stats?.remote||0,color:'#60a5fa',bg:'rgba(96,165,250,.08)'},
          {label:'Away',value:stats?.away||0,color:'#9e9b94',bg:'rgba(0,0,0,.04)'},
          {label:'Total Active',value:stats?.total_active_users||0,color:'#22c55e',bg:'rgba(34,197,94,.08)'},
        ].map(s=><div key={s.label} style={{background:'#fff',border:'1px solid #f0ede8',borderRadius:14,padding:'14px 16px'}}><div style={{fontSize:10,fontWeight:700,color:'#9e9b94',marginBottom:6}}>{s.label}</div><div style={{fontSize:26,fontWeight:900,color:s.color}}>{s.value}</div></div>)}
      </div>
      {/* Weekly chart */}
      {weekly.length>0&&<div style={{background:'#fff',border:'1px solid #f0ede8',borderRadius:16,padding:'20px'}}>
        <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:900,marginBottom:16}}>This Week</div>
        <div style={{display:'flex',gap:8,alignItems:'flex-end',height:100}}>
          {weekly.map(d=>{const total=d.office+d.remote+d.away||0;const h=total?Math.max((total/maxVal)*80,4):4;return(
            <div key={d.day} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
              <div style={{width:'100%',height:h,background:'#f97316',borderRadius:'4px 4px 0 0',transition:'height .4s'}}/>
              <span style={{fontSize:10,fontWeight:600,color:'#9e9b94'}}>{d.day}</span>
            </div>
          );})}
        </div>
      </div>}
      {/* Task stats */}
      {taskStats&&<div style={{background:'#fff',border:'1px solid #f0ede8',borderRadius:16,padding:'20px'}}>
        <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:900,marginBottom:14}}>Task Overview</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10}}>
          {[{l:'Total',v:taskStats.total,c:'#0f0e0c'},{l:'To Do',v:taskStats.todo,c:'#9e9b94'},{l:'In Progress',v:taskStats.in_progress,c:'#f97316'},{l:'Done',v:taskStats.done,c:'#22c55e'}].map(s=><div key={s.l} style={{textAlign:'center',padding:'12px 8px',background:'#faf9f7',borderRadius:10}}><div style={{fontSize:20,fontWeight:900,color:s.c}}>{s.v}</div><div style={{fontSize:10,fontWeight:600,color:'#9e9b94',marginTop:2}}>{s.l}</div></div>)}
        </div>
      </div>}
    </div>
  );
