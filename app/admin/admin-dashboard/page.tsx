                ...
                {tasks.map(t=>(
                  <div key={t.id} style={{...S.card,padding:'10px 16px',display:'flex',alignItems:'center',gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:12,color:'#888',marginTop:1}}>{t.assigned_to_name||'Team member'}</div>
                    </div>
...
function ManageSection({user,showToast}:{user:User|null;showToast:(m:string,t?:'success'|'error')=>void}) {
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {members.map(m=>(
          <div key={m.id} style={{background:'#fff',border:'1.5px solid #ebebeb',borderRadius:12,padding:'10px 16px',display:'flex',alignItems:'center',gap:12,boxShadow:'0 1px 4px rgba(0,0,0,.04)'}}>
